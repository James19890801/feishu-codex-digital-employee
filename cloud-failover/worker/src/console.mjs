...
</style></head><body><main class="wrap ${tone}"><div class="eyebrow">AIPR0S · LOCAL FIRST · CLOUDFLARE</div><h1>云端兜底控制台</h1><p class="sub">本地为主，Qoder Cloud Agent 仅处理经过脱敏的 L0/L1 文本。</p>
...
<section class="grid"><article class="card"><span class="label">心跳距离现在</span><strong class="value">${escapeHtml(formatAge(status.heartbeatAgeMs))}</strong></article><article class="card"><span class="label">待机容器</span><strong class="value">${status.containerReady ? 'READY' : 'STANDBY'}</strong></article><article class="card"><span class="label">云端处理中</span><strong class="value">${escapeHtml(status.inFlight || 0)}</strong></article></section>
