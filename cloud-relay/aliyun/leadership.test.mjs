...
test('three healthy recovery heartbeats drain cloud before a newer local generation', () => {
  ...
  try {
    ...
    assert.equal(store.recoveryHeartbeat({ now: 92_000, healthy: true }).state, 'CLOUD_ACTIVE');
    assert.equal(store.recoveryHeartbeat({ now: 93_000, healthy: true }).state, 'DRAINING');
    assert.deepEqual(store.finishCloudDrain({ now: 94_000 }), {
...
test('leadership survives database reopen and never reuses a generation', () => {
  ...
  try {
    ...
    reopened.recoveryHeartbeat({ now: 92_000, healthy: true });
    reopened.recoveryHeartbeat({ now: 93_000, healthy: true });
    assert.equal(reopened.finishCloudDrain({ now: 94_000 }).generation, 3);
    reopened.close();
  } finally { rmSync(directory, { recursive: true, force: true }); }
});
