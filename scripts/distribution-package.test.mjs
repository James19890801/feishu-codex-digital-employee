...
const fixture = await mkdtemp(join(tmpdir(), 'aipro-package-source-'));
...
await put(fixture, 'package.json', '{"name":"fixture","version":"1.0.0"}\n');
...
await put(fixture, 'config.local.json', '{"token":"private"}\n');
...
const dirty = await mkdtemp(join(tmpdir(), 'aipro-package-dirty-'));
...
const outputDir = await mkdtemp(join(tmpdir(), 'aipro-package-output-'));
