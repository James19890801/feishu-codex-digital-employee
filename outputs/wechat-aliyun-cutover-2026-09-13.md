...
- 服务器当前到期日：2026-12-21 23:59:59（阿里云控制台核验）。新域名证书到期：2026-12-11；`certbot-renew.timer` 已启用，`certbot renew --dry-run` 成功；续签后仅针对新域名校验证书并重载 Nginx 的 hook 已安装和手动验证。
...
- 切换约 5 分钟后复核：本机微信中继代理 `ok=true`，微信与钉钉仍健康，新旧队列继续为 0。
