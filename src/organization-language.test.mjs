...
for (const [text, expected] of positiveCases) {
  const result = annotateOrganizationLanguage(text);
...
const ambiguous = annotateOrganizationLanguage('按7来');
assert.deepEqual(ambiguous.annotations, []);
assert.equal(ambiguous.ambiguous, true);

const contextual = annotateOrganizationLanguage('按7来', [
  { direction: 'counterparty', content: '这个岗位定什么层级？' },
]);
assert.match(contextual.annotations.join('\n'), /P7/);
assert.equal(contextual.ambiguous, false);

const glossary = annotateOrganizationLanguage('这个链路的卡点要对焦，明确 owner 后倒排，最后复盘沉淀');
...
const formatted = formatOrganizationLanguageAnnotations(annotateOrganizationLanguage('这个同学是6，要拿结果并闭环'));
assert.match(formatted, /企业语境注释/);
assert.match(formatted, /P6/);
assert.match(formatted, /拿结果/);
assert.match(formatted, /闭环/);

const none = formatOrganizationLanguageAnnotations({ annotations: [], ambiguous: false });
assert.equal(none, '企业语境注释：\n（本轮没有需要补充的企业语义）');

console.log('ORGANIZATION_LANGUAGE_TEST_OK');
