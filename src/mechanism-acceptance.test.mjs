...
contract('local-wiki', 'Do all IM channels share one evidence-gated local knowledge context?', async () => {
  ...
  const contexts = await Promise.all(['feishu', 'enterpriseChat', 'wechat'].map(channel => (
...
contract('wechat-owner-article-syndication', 'Does the live WeChat runtime detect and syndicate exact owner articles?', () => {
  const runtimeSource = readFileSync(new URL('./index.mjs', import.meta.url), 'utf8');
  assert.match(runtimeSource, /import \{ WeChatOwnerArticleSyndication \} from '\.\/wechat-owner-article-syndication\.mjs'/);
...
contract('multimodal-pipeline', 'Can a EnterpriseChat group file placeholder become a downloadable drive reference?', () => {
  const file = parseEnterpriseChatFilePlaceholder('[文件] 复盘.pptx fileId: drive-node-1 注意：如需下载使用connector drive download命令下载');
  assert.equal(file.fileName, '复盘.pptx');
  assert.ok(buildEnterpriseChatDriveDownloadArgs({
...
for (const channel of ['feishu', 'enterpriseChat', 'unknown']) {
      ...
      for (const actor of ['owner', 'other']) {
        contract('owner-authorization',
          `${channel}/${chatType}/self=${selfChat}/${actor} can write Multica?`, () => {
            const senderId = channel === 'enterpriseChat'
              ? `enterpriseChat:${actor === 'owner' ? 'dt_owner' : 'dt_other'}`
              : actor === 'owner' ? 'ou_owner' : 'ou_other';
            const expected = actor === 'owner'
              && chatType === 'p2p'
              && selfChat
              && ['feishu', 'enterpriseChat'].includes(channel);
...
for (const [question, input, expected] of [
  ['Do EnterpriseChat group discussions use adaptive budgeting?', {
    enabled: true, channel: 'enterpriseChat', chatType: 'group', messageType: 'text', text: '为什么会这样？',
  }, true],
  ...
  ['Do direct messages bypass adaptive budgeting?', {
    enabled: true, channel: 'enterpriseChat', chatType: 'p2p', messageType: 'text', text: '为什么会这样？',
  }, false],
]) {
...
contract('human-takeover', 'Can an assistant echo accidentally extend human takeover?', () => {
  const applied = applyOwnerActivityHistory([{
    ...
    openMessageId: 'owner-1', senderEnterpriseUserId: 'owner',
  }, {
    content: '助理回复', createTime: '2026-08-02 08:01:00',
    openMessageId: 'assistant-1', senderEnterpriseUserId: 'owner',
  }], {
    ownerId: 'owner',
...
contract('human-takeover', 'Does normal Owner activity stop only a direct conversation?', () => {
  const messages = [{
    ...
    openMessageId: 'owner-direct', senderEnterpriseUserId: 'owner',
  }];
  const options = {
    ownerId: 'owner',
...
for (const [question, input, expected] of [
  ['Does a EnterpriseChat group reply @ the requester?', {
    chatId: 'enterpriseChat:group:cid-1', chatType: 'group',
    senderId: 'enterpriseChat:requester-1', text: '已处理',
  ...
  ['Does a direct reply avoid accidental @?', {
    chatId: 'enterpriseChat:user:requester-1', chatType: 'p2p',
    senderId: 'enterpriseChat:requester-1', text: '已处理',
  }, { prefix: '已处理', at: [] }],
  ['Does a missing sender ID degrade without a malformed @?', {
    chatId: 'enterpriseChat:group:cid-1', chatType: 'group', senderId: '', text: '已处理',
  }, { prefix: '已处理', at: [] }],
]) {
  contract('group-attribution', question, () => {
    ...
    assert.deepEqual(result.mentionUserIds, expected.at);
  });
}

contract('group-attribution', 'Are EnterpriseChat mentions deduplicated and bounded?', () => {
  const ids = Array.from({ length: 25 }, (_, index) => `user-${index}`);
  const text = ids.map(id => `<@${id}>`).join(' ') + '\n通知';
  const args = buildEnterpriseChatSendArgs(
    { channel: 'enterpriseChat', kind: 'group', id: 'cid-1' },
    text,
    'bounded-mentions',
...
contract('group-attribution', 'Can a missing EnterpriseChat @ placeholder pass silently?', () => {
  assert.throws(() => buildEnterpriseChatSendArgs(
    { channel: 'enterpriseChat', kind: 'group', id: 'cid-1' },
    '没有占位符',
    'missing-placeholder',
    { mentionUserIds: ['requester-1'] },
  ), /mention placeholder/i);
});

for (const [question, input, expected] of [
  ['Do EnterpriseChat group text messages enter semantic loop protection?', {
    enabled: true, channel: 'enterpriseChat', chatType: 'group', messageType: 'text', text: '继续讨论',
  }, true],
  ...
  ['Do direct messages remain unchanged?', {
    enabled: true, channel: 'enterpriseChat', chatType: 'p2p', messageType: 'text', text: '继续讨论',
  }, false],
  ...
  ['Do media-only messages remain unchanged?', {
    enabled: true, channel: 'enterpriseChat', chatType: 'group', messageType: 'image', text: '',
  }, false],
]) {
  contract('loop-prevention', question, () => {
...
for (const [question, event, accepted] of [
  ...
  contract('inbound-normalization', question, () => {
    assert.equal(Boolean(normalizeEnterpriseChatEvent(event)), accepted);
  });
}
...
for (const [chatId, request] of [
  ['enterpriseChat:user:u1', '请给我出一份方案'],
  ['enterpriseChat:group:c1', '做一个数据分析表格'],
  ['oc_feishu', '整理一份项目复盘'],
]) {
...
contract('explicit-mention-priority', 'Does repeat suppression acknowledge a required response?', async () => {
  await withAsyncState('aipro-acceptance-explicit-repeat-', async state => {
    ...
    const base = {
      ...
      channel: 'enterpriseChat', senderId: 'member-a',
      message: {
        message_id: 'repeat-1', chat_id: 'enterpriseChat:group:test',
...
contract('explicit-mention-priority', 'Does discussion cooldown acknowledge a required response?', async () => {
  await withAsyncState('aipro-acceptance-explicit-discussion-', async state => {
    const sent = [];
    const baseMessage = {
      message_id: 'discussion-start', chat_id: 'enterpriseChat:group:explicit',
      chat_type: 'group', message_type: 'text',
    };
    const base = {
      state, enabled: true, maxReplies: 100, lowValueLimit: 1,
      cooldownMs: 30 * 60_000, channel: 'enterpriseChat', message: baseMessage,
      sendClose: async (text, key) => sent.push({ text, key }),
    };
...
contract('group-host-mode', 'Does another member picking up the topic prevent host output?', async () => {
  ...
  const result = await processGroupHostCandidate({
    ...
    recentMessages: [{
      role: 'user',
      senderId: 'enterpriseChat:member-b',
...
contract('group-host-mode', 'Does a recovered send failure produce only one successful host reply?', async () => {
  await withAsyncState('aipro-acceptance-group-host-retry-', async state => {
    ...
    state.scheduleGroupHostCandidate({
      messageId: 'host-retry-1', chatId: 'enterpriseChat:group:test',
      senderId: 'enterpriseChat:member-a', text, topic: assessment.topic,
...
contract('group-host-mode', 'Does runtime allowlist removal suppress already-queued work?', async () => {
  ...
  const result = await processGroupHostCandidate({
    candidate: {
      messageId: 'host-removed-1', senderId: 'enterpriseChat:member-a',
...
contract('group-host-mode', 'Does recent activity use a no-penalty durable defer transition?', () => {
  withState('aipro-acceptance-group-host-defer-', state => {
    ...
    state.scheduleGroupHostCandidate({
      messageId: 'host-defer-1', chatId: 'enterpriseChat:group:test',
      senderId: 'enterpriseChat:member-a', text, topic: assessment.topic,
      createdAtMs: 1_000, dueAtMs: 2_000,
    });
...
contract('group-host-mode', 'Does worker recovery keep private text out of retry records?', async () => {
  ...
  const result = await runGroupHostWorkerIteration({
    nowMs: 10_000,
    claim: () => ({
      messageId: 'host-private-1', chatId: 'enterpriseChat:group:test',
      senderId: 'enterpriseChat:member-a', text: privateText, attempts: 1,
    }),
...
contract('group-host-mode', 'Can the allowlisted EnterpriseChat group recover messages missed by the event stream?', () => {
  const runtimeSource = readFileSync(new URL('./index.mjs', import.meta.url), 'utf8');
  assert.match(runtimeSource, /runEnterpriseChatGroupHostRecoveryLoop/);
  assert.match(runtimeSource, /normalizeEnterpriseChatGroupHistoryMessages/);
  assert.match(runtimeSource, /ownerOpenId:\s*config\.enterpriseChatOwnerOpenId/);
  assert.match(runtimeSource, /enqueueInbound\(payload, 'enterpriseChat-group-host-recovery'\)/);
});
