...
assert.deepEqual(unconfigured.map(item => item.id), ['codex', 'qoder', 'codebuddy', 'trae', 'ai-lab']);
...
const result = await client.run('私密提示词', { cwd: '/tmp/ignored', timeoutMs: 30_000 });
...
assert.equal(requests[0].options.headers.buc_user, JSON.stringify({ workNo: '384351' }));
...
assert.throws(() => aiRuntime.normalizeAiLabRuntimeConfiguration({ ...normalized, endpoint: 'http://example.com' }), /HTTPS endpoint/i);
assert.throws(() => aiRuntime.normalizeAiLabRuntimeConfiguration({ ...normalized, agentId: 'wrong' }), /Agent ID/i);
assert.throws(() => aiRuntime.normalizeAiLabRuntimeConfiguration({ ...normalized, apiKey: 'wrong' }), /API Key/i);
assert.throws(() => aiRuntime.normalizeAiLabRuntimeConfiguration({ ...normalized, workNo: 'abc' }), /work number/i);
