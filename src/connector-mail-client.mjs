import { isSupportedConnectorExecutable } from './conversation-context-client.mjs';
...
export class ConnectorMailError extends Error {
  constructor(message, code = 'CONNECTOR_MAIL_UNAVAILABLE', options = {}) {
    super(message, options);
    this.name = 'ConnectorMailError';
    this.code = code;
  }
}
...
function required(value, label) {
  ...
  if (!text) throw new ConnectorMailError(`CONNECTOR mail ${label} is required`, 'CONNECTOR_MAIL_INVALID_INPUT');
  return text;
}
...
function boundedLimit(value) {
  ...
  if (!Number.isInteger(parsed) || parsed < 1) {
    throw new ConnectorMailError('CONNECTOR mail limit must be a positive integer', 'CONNECTOR_MAIL_INVALID_INPUT');
  }
...
function normalizeMailboxes(root) {
  ...
  if (!Array.isArray(items)) {
    throw new ConnectorMailError('CONNECTOR mailbox response has no account list', 'CONNECTOR_MAIL_INVALID_RESPONSE');
  }
...
function normalizeSearch(root) {
  ...
  if (!Array.isArray(items)) {
    throw new ConnectorMailError('CONNECTOR mail search response has no message list', 'CONNECTOR_MAIL_INVALID_RESPONSE');
  }
  return {
...
function normalizeMessage(root) {
  ...
  if (!item || typeof item !== 'object') {
    throw new ConnectorMailError('CONNECTOR mail response has no message', 'CONNECTOR_MAIL_INVALID_RESPONSE');
  }
  const id = String(item.id || item.messageId || '').trim();
  if (!id) throw new ConnectorMailError('CONNECTOR mail response has no message id', 'CONNECTOR_MAIL_INVALID_RESPONSE');
  return {
    id,
    subject: String(item.subject || '').trim(),
    from: address(item.from || item.sender),
...
function normalizeUsers(root) {
  ...
  if (!Array.isArray(items)) {
    throw new ConnectorMailError('CONNECTOR user response has no user list', 'CONNECTOR_MAIL_INVALID_RESPONSE');
  }
  return items.map(item => ({
...
export function normalizeDeliveryStatus(root) {
  const value = String(
    root?.result?.message?.deliveryStatus
    || root?.message?.deliveryStatus
    || root?.result?.deliveryStatus
    || root?.deliveryStatus
    || '',
  ).trim().toLowerCase();
...
function safeCategory(error) {
  if (error instanceof ConnectorMailError) return error.code;
  const detail = String(error?.message || error || '').toLowerCase();
  return /timeout|timed out/.test(detail) ? 'CONNECTOR_MAIL_TIMEOUT' : 'CONNECTOR_MAIL_PROCESS_FAILED';
}

export class ConnectorMailClient {
  constructor({
    bin,
    profile,
    transport,
  ...
  _validate() {
    if (!isSupportedConnectorExecutable(this.bin)) {
      throw new ConnectorMailError(
        'Mail requires the configured standalone CONNECTOR installation',
        'CONNECTOR_MAIL_PATH_REJECTED',
      );
    }
    if (this.transport !== 'event-stream') {
      throw new ConnectorMailError(
        'Mail requires the existing CONNECTOR event-stream transport',
        'CONNECTOR_MAIL_TRANSPORT_REJECTED',
      );
    }
    if (typeof this.runner !== 'function') {
      throw new ConnectorMailError('CONNECTOR mail runner is unavailable');
    }
  }
  ...
  async _run(action, args, normalize) {
    ...
    try {
      ...
      const wrapped = error instanceof ConnectorMailError
        ? error
        : new ConnectorMailError(
            `CONNECTOR mail process failed: ${String(error?.message || error)}`,
            safeCategory(error),
            { cause: error },
          );
      this.audit(`connector_mail_${action}_failed`, {
        durationMs: Date.now() - startedAt,
        status: 'failed',
  ...
  sendMessage({ from, to = [], cc = [], subject, content } = {}) {
    ...
    if (!recipientsTo.length) {
      throw new ConnectorMailError('CONNECTOR mail recipients are required', 'CONNECTOR_MAIL_INVALID_INPUT');
    }
  ...
  forwardMessage({ from, id, to = [], content = '', subject = '' } = {}) {
    ...
    if (!recipientList.length) throw new ConnectorMailError('CONNECTOR mail recipients are required', 'CONNECTOR_MAIL_INVALID_INPUT');
