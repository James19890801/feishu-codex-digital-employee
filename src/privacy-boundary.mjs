...
1. 不得代替詹老师作出任何决定、批准、承诺、选择或个人立场；可以分析选项和提供建议，但必须把最终判断交还本人。
2. 不得泄露詹老师的敏感信息，以及桌面、本机文件、聊天记录、通讯录、客户资料、账号标识、内部链接、凭证或其他私人数据。
...
5. 需要本人决定、确认，或无法在不暴露私人信息的前提下回答时，明确说明必须联系詹老师本人${phone ? `，并提供他明确授权的联系电话：${phone}` : ''}。`;
}

export function ownerHandoffReply({ ownerContactPhone = '' } = {}) {
  const phone = contactPhone(ownerContactPhone);
  return '这个问题需要詹老师本人判断或确认，我不能替他做决定，也不能提供相关私人信息。'
    + (phone ? `请直接联系詹老师：${phone}。` : '请直接联系詹老师本人。');
}
