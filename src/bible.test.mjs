...
assert.deepEqual(decideWorkflow('帮忙创建需求，我们一起来搞定群聊场景下，数字人如何适当响应').intent, 'multica_issue');
assert.deepEqual(decideWorkflow('帮忙创建需求，我们一起来搞定群聊场景下，数字人如何适当响应').action, 'preview_confirm');
assert.deepEqual(decideWorkflow('更新 MYS-2 的状态为完成').action, 'preview_confirm');
assert.deepEqual(decideWorkflow('@詹老师 这个内容太长，给我一个pdf或者word吧').intent, 'artifact');
assert.deepEqual(decideWorkflow('@詹老师 这个内容太长，给我一个pdf或者word吧').action, 'preview_confirm');
