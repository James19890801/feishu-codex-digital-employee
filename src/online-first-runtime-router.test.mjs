...
function runtimeClient(id, run) {
  return {
    runtime: { id, label: id === 'ai-lab' ? 'AI-Lab Agent（预发）' : 'Codex CLI' },
    run,
  };
}
...
let onlineCalls = 0;
let localCalls = 0;
const onlineFailure = new Error('AI-Lab Agent failed: NETWORK_ERROR');
...
assert.equal(onlineCalls, 1, 'an open circuit must not repeatedly call the failing online runtime');
assert.equal(localCalls, 2);

now += 30_000;
await router.run('熔断结束后重新探测线上');
assert.equal(onlineCalls, 2, 'the online runtime must be probed again after the circuit cooldown');
assert.equal(localCalls, 3);

let permissionFallbackCalls = 0;
const permissionRouter = new OnlineFirstRuntimeRouter({
  onlineClient: runtimeClient('ai-lab', async () => {
    throw new Error('AI-Lab Agent failed: HTTP_401');
  }),
  localClient: runtimeClient('codex', async () => {
...
await assert.rejects(
  () => permissionRouter.run('权限问题不能被降级掩盖'),
  /HTTP_401/,
);
...
const onlineSuccessRouter = new OnlineFirstRuntimeRouter({
  onlineClient: runtimeClient('ai-lab', async () => ({
    text: '线上回复',
    runtime: { id: 'ai-lab', label: 'AI-Lab Agent（预发）' },
  })),
  localClient: runtimeClient('codex', async () => {
    throw new Error('online success must not call local');
  }),
});
...
const imageRouter = new OnlineFirstRuntimeRouter({
  ...
  localClient: {
    ...
    async run() {
      return { text: '本地图片回复', runtime: this.runtime };
    },
  },
});
...
assert.equal(imageOnlineCalls, 0, 'an image must not be silently dropped by the online runtime');
