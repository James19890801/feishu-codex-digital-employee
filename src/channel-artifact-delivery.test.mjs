...
const enterpriseChat = buildChannelArtifactDeliveryPlan({
  channel: 'enterpriseChat',
  chatId: 'enterpriseChat:group:cid-1',
  target: { channel: 'enterpriseChat', kind: 'group', id: 'cid-1' },
  path: '/workspace/outputs/demo.mp4',
  relativePath: 'outputs/demo.mp4',
  caption: '视频已生成',
  idempotencyKey: 'artifact-2',
});
assert.equal(enterpriseChat.channel, 'enterpriseChat');
assert.deepEqual(enterpriseChat.attachmentArgs.slice(0, 7), [
  'chat', 'message', 'send', '--group', 'cid-1', '--msg-type', 'file',
]);
assert.equal(enterpriseChat.caption, '视频已生成');
