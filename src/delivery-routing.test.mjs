...
const requests = [
  ['enterpriseChat:user:open-user', '请给我出一份招聘优化方案'],
  ['oc_feishu_chat', '整理一份项目复盘报告'],
  ['enterpriseChat:group:cid', '做一个招聘数据分析表格'],
...
for (const [chatId, request, formats] of [
  ['enterpriseChat:user:u1', '最后生成一份 Word 方案给我', ['docx']],
  ['oc_feishu_chat', '把报告做成PDF附件', ['pdf']],
  ['oc_feishu_chat', '最终给我PDF和Excel文件', ['pdf', 'xlsx']],
  ['enterpriseChat:group:cid', '把完整页面作为HTML文件发到群里', ['html']],
]) {
  const plan = buildDeliveryPlan({ chatId, request });
  assert.equal(plan.kind, 'artifact');
  assert.deepEqual(plan.formats, formats);
  assert.equal(plan.provider, chatId.startsWith('enterpriseChat:') ? 'enterpriseChat' : 'feishu');
