...
| Railway standby CONNECTOR runtime | Live, coordinator-confirmed | Independent CONNECTOR authorization, always-warm event stream, mention-only 3-minute backfill, generation fencing and UUID send; Cloudflare readback reached `CLOUD_ACTIVE` with runtime `ACTIVE` |
| Railway image build | Live on Railway | Pinned `linux/amd64` Node image, CA certificates and CONNECTOR CLI layer build successfully; Cloudflare Container Registry is no longer required |
...
| Live Railway EnterpriseChat | Activated with the local-main blacklist | Isolated CONNECTOR device authorization is persisted on the Railway volume; the registered routing Channel is injected into every CONNECTOR command; the local Profile is not copied. Full 7x24 acceptance still requires a controlled 30-minute Mac outage and one real late-message reply/readback |
...
2. **Whole-host takeover:** if the Mac cannot send heartbeats, three missed 30-second heartbeats move the coordinator to `TAKING_OVER`; Railway confirms its CONNECTOR event stream and moves it to `CLOUD_ACTIVE`. Three healthy local heartbeats start draining and return ownership to `LOCAL_PRIMARY`. The controlled 30-minute outage is an acceptance test, not a takeover delay.

If the per-message cloud gateway is unavailable, the local durable inbox leaves the message retryable rather than marking it complete. EnterpriseChat sends use a stable message UUID, while the Cloudflare `handoffId` prevents duplicate Qoder generation across process and network retries.

When the active local service cannot yet be switched to this isolated branch, install the independent macOS heartbeat sidecar with `./scripts/install-cloud-failover-heartbeat-sidecar.sh`. It reads only the metadata-only local `/api/status`, stores no local content, and signs a healthy heartbeat only when the process, AI runtime and CONNECTOR channel are all healthy. An unhealthy heartbeat does not move the coordinator's last-healthy boundary, so three intervals still trigger Railway. Remove the sidecar after the integrated heartbeat is deployed to the active local service; never run both as separate authorities long term.
...
- Cloud accepts L0/L1 text and bounded EnterpriseChat images. Images are downloaded by the independently authorized Railway CONNECTOR identity, validated as a regular PNG/JPEG/GIF/WebP file of at most 4 MiB, described through the Cloudflare Workers AI binding, and passed to Qoder only as bounded text. Audio, video, ordinary files, mail, documents, repository content, local memory, credentials, L2 and L3 fail closed.
...
- Dedicated, revocable EnterpriseChat OAuth authorization for cloud standby. CONNECTOR built-in device OAuth is the default; a self-created EnterpriseChat app is optional. Do not export the configured local CONNECTOR Profile. Organization commands must carry the same registered digital-human Channel code used during device login.
...
AIPROS_CLOUD_CONNECTOR_CHANNEL
AIPROS_CONNECTOR_HOME
AIPROS_COORDINATOR_URL
AIPROS_CONTAINER_TOKEN
AIPROS_ACCESS_MODE
AIPROS_BLOCKED_CHAT_IDS
AIPROS_BLOCKED_SENDER_IDS
RAILWAY_RUN_UID
```

Set `AIPROS_ACCESS_MODE=blacklist`. Synchronize `AIPROS_BLOCKED_SENDER_IDS` from the active local main configuration's EnterpriseChat `automaticCommunicationBlocklist`; `AIPROS_BLOCKED_CHAT_IDS` is optional for conversation-level blocks. Normal direct messages and @-mentioned group messages are accepted unless a sender or conversation is blocked. Display names are never enforcement identities.

`ENTERPRISE_CHAT_CLIENT_ID` and `ENTERPRISE_CHAT_CLIENT_SECRET` are optional overrides for a self-created app. Omit both to use CONNECTOR built-in device OAuth; supplying only one fails closed.
...
## Independent cloud CONNECTOR authorization

Use a dedicated CONNECTOR state on an isolated operator session or machine. AppKey/AppSecret alone cannot log in as a person.

```zsh
connector auth login --device

connector auth status --format json

connector auth export --base64 > connector-auth.b64
```

Run these commands with CONNECTOR 1.0.56, `CONNECTOR_DISABLE_KEYCHAIN=1`, the registered digital-human `CONNECTOR_CHANNEL`, and an isolated `HOME`; the user completes one device authorization in EnterpriseChat. Before export, verify that this state contains only the dedicated cloud authorization and is not the local production Profile. Store the base64 output as `ENTERPRISE_CHAT_CONNECTOR_AUTH_BUNDLE_B64`, and place the same public routing code in `AIPROS_CLOUD_CONNECTOR_CHANNEL`, then securely delete the export file. The Railway runtime imports the bundle once with mode `0600` into the persistent `AIPROS_CONNECTOR_HOME`, injects that Channel only into CONNECTOR subprocesses, verifies a real `connector auth status`, and records a bootstrap marker. Later restarts use the persisted, rotated credential state and fail closed instead of re-importing the original bundle. Mount a Railway Volume at `/data` and set `RAILWAY_RUN_UID=0`, because Railway mounts volumes as root.
...
`cloud-failover:dry` validates the Worker bundle without any Container Registry access. Railway detects `cloud-failover/container/Dockerfile` when that directory is deployed as the service root. The committed Railway configuration uses `/live`, zero deployment overlap, 30-second draining and the `ALWAYS` restart policy. Zero overlap is required because one dedicated CONNECTOR identity supports one active event consumer; generation fencing and the mention-only backfill cover the short replacement window. Deployment acceptance must read the policy back from Railway; if the account rejects it, retain `ON_FAILURE` and do not claim verified 7x24 operation.
...
3. Send a harmless authorized EnterpriseChat message and confirm the local reply has no cloud label.
...
8. Confirm no duplicate cloud/local reply and verify the stable CONNECTOR UUID plus claim ledger.

Only after all eight checks have current evidence may the deployment be described as 7x24 verified.

## Rollback

Set `cloudFailoverEnabled` to `false` and restart the local service. Disable Railway lease activation, stop the Railway service, and revoke the dedicated EnterpriseChat authorization and Railway coordinator token. Delete the Cloudflare Worker only if per-request Qoder fallback is also being retired. The local CONNECTOR Profile and local SQLite are never copied; the static Channel code is only reused as an Organization organization routing header.
